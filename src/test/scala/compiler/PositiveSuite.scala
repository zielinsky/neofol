...
class PositiveSuite extends munit.FunSuite:

  private val compiler = Compiler(Options())
  private val root = Path.of("tests", "pos")
  ...
  programs.foreach { path =>
    ...
    test(s"$relativePath compiles without diagnostics"):
      val compilation =
      ...
      assertEquals(compilation.diagnostics, Vector.empty)
  }
